...
float3 GetDetail(InputConfig c)
{
	if (c.useDetail)
	{
		float4 map = SAMPLE_TEXTURE2D(_DetailMap, sampler_BaseMap, c.detailUV);
		return map.rgb * 2.0 - 1.0;
	}
...
float4 GetBase(InputConfig c) {
	...
	if (c.useDetail)
	{
		float3 detail = GetDetail(c).r * INPUT_PROP(_DetailAlbedo);
...
float GetOcclusion(InputConfig c)
{
	...
	return lerp(GetMask(c).g, 1.0, strength);
}
...
float3 GetNormalTS(InputConfig c)
{
	...
	if (c.useDetail)
	{
		map = SAMPLE_TEXTURE2D(_DetailNormalMap, sampler_BaseMap, c.detailUV);
